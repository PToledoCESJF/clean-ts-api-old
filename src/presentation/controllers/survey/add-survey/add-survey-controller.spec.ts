...
describe('AddSurvey Controller', () => {
  ...
  test('Sould return 400 if Validation fails', async () => {
    const { sut, validationStub } = makeSut()
  ...
  test('Sould call AddSurvey with correct values', async () => {
  ...
  test('Sould return 500 if AddSurvey throws', async () => {
    const { sut, addSurveyStub } = makeSut()
  ...
  test('Sould return 204 on success', async () => {
    const { sut } = makeSut()
    const httpResponse = await sut.handle(makeFakeRequest())
